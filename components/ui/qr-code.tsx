...
function QRCodeCanvas(props: QRCodeCanvasProps) {
  ...
  return (
    <CanvasPrimitive
      ...
      width={context.size}
      height={context.size}
      className={cn('relative max-h-(--qr-code-size) max-w-(--qr-code-size)', !generationKey && 'invisible', className)}
    />
  );
}
...
function QRCodeDownload(props: QRCodeDownloadProps) {
  const { filename = 'qrcode', format = 'png', asChild, className, children, ...buttonProps } = props;
...
function QRCodeSkeleton(props: QRCodeSkeletonProps) {
  ...
  return (
    <SkeletonPrimitive
      ...
      {...skeletonProps}
      className={cn('max-h-(--qr-code-size) max-w-(--qr-code-size) animate-pulse bg-muted', className)}
      style={{
        width: context.size,
        height: context.size,
        ...style,
